                ...
                <div className="bg-white rounded-2xl shadow-sm overflow-hidden mb-8 mt-22">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-6 lg:ml-10 lg:mr-12">
                        ...
                        <div className="space-y-7">
                            <div className="aspect-square relative bg-gray-50 rounded-xl overflow-hidden w-full max-w-[500px] max-h-[500px]"> {/* Adjust max width and height */}
                                <img
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="flex items-center gap-3">
                                <div className="flex items-center border border-gray-300 rounded-lg">
                                    ...
                                    <span className="px-4 py-2 font-medium min-w-[60px] text-center">{quantity}</span> {/* Reduced padding and width */}
                                    <button
                                        onClick={() => handleQuantityChange(1)}
                            ...
                            {/* Product Description */}
                            <div className="border-t border-gray-100 pt-4"> {/* Reduced top padding */}
                                <h3 className="font-semibold text-green-600 mb-3 text-sm">Deskripsi Produk</h3> {/* Reduced font size and margin */}
                                <ul className="space-y-2 mb-3"> {/* Reduced space between list items */}
                                    {product.description.map((item, index) => (
                                        <li key={index} className="flex items-start gap-2 text-gray-700"> {/* Reduced gap */}
                                            <span className="w-2 h-2 bg-green-500 rounded-full mt-1 flex-shrink-0"></span> {/* Reduced margin top */}
                                            <span>{item}</span>
                                        </li>
                                    ))}
                                </ul>

                                <p className="text-gray-700 mb-3 leading-relaxed text-sm">{product.fullDescription}</p> {/* Reduced bottom margin and font size */}

                                <div className="bg-gray-50 rounded-lg p-3 mb-3"> {/* Reduced padding and margin */}
                                    <div className="text-xs text-gray-600"> {/* Reduced font size */}
                                        <span className="font-medium">No. Sertifikat Halal:</span>
                                        <span className="ml-2 font-mono">{product.certificateNumber}</span>
                                    </div>
                                </div>

                                <button className="text-green-600 hover:text-green-700 font-medium text-sm transition-colors"> {/* Reduced font size */}
                                    Lihat selengkapnya →
                                </button>
                            </div>
                        </div>
